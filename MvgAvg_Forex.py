...
def analyzeStock(stock):
    ...
    resultsFile = open('outputResultsForex.txt','a')
    
    #results = quotes.groupby(['DaysAgo']).apply(resultsPerDay)
    results = sum(quotes.Profit)
    print 'Analysis for %s:' % (stock) 
    print results
    
    resultsFile.write('\nAnalysis for ' + stock + ':\n')
    resultsFile.write(str(results) + '\n')
    resultsFile.write('Total profits: ' + str(results) + '\n')
    resultsFile.close()

stocksToAnalyze = 'USDEUR=X','EURUSD=X'

createResultsFile = open('outputResultsForex.txt','w')
